...
(function ($) {
  ...
  $.fn.kcfinder = function (options) {
    ...
    for (i in parse) {
      i = parse[i];
      if (o[i].length) url += i + "=" + encodeURIComponent(o[i]) + "&";
    }
    ...
    if ($.isFunction(o.callback) || $.isFunction(o.callbackMultiple)) {
      if (!window.KCFinder) window.KCFinder = {};

      // Single file callback
      if ($.isFunction(o.callback)) window.KCFinder.callBack = o.callback;
      ...
      // No callbacks
    } else if (window.KCFinder) delete window.KCFinder;
  };
})(jQuery);
